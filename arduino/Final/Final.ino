...
//Variables para el calculo del paracaidas
int state = 0; /*0 subiendo, 1 arriba, 2 callendo**/
float sum = 0;
float altura=0;
float velocidad=0;
float aceleracion_total=0;
...
float old_altitud = 0;
  ...
  float altitud = 0;
float latitud = 0.0; //2
float longitud = 0.0; //4
char latitud_dir = 'N'; //3
char longitud_dir = 'N'; //5
float gps_altitud = 0.0; //9
void setup()
{
  char rtn = 0;
    Serial.begin(9600);
  ...
  {
    Radio.println("#LSM303D is not found;");
    while(1);
  }
  else
  {
    Radio.println("#LSM303D is found;");
  }
  delay(150);
  /* Reset HP20x_dev */
  //HP20x.begin();
...
int estado = 0;
void TestParachute(){
  if(estado == 0 && (abs(realAccel[0])+abs(realAccel[1])+abs(realAccel[2])) >= 3){
    estado = 1; //El cohete ha despegado
  }
  else if(estado  == 1){
    if(old_altitud == 0){
      old_altitud = altitud;
    }else if(altitud <= (old_altitud + 1)){
      servoMotor.write(180);
      estado = 2; //El paracaidas se ha desplegado
      Radio.println("DEPLOY");
      Radio.println("DEPLOY");
      Radio.println("DEPLOY");
      Radio.println("DEPLOY");
      Radio.println("DEPLOY");
...
void GetGps(){
  //$GPGGA,150032.000,3711.8590,N,00336.8736,W,1,3,26.56,603.8,M,49.
  ...
  if (SoftSerial.available())                     // if date is coming from software serial port ==> data is coming from SoftSerial shield
  {
       while(SoftSerial.available())               // reading data into char array
        ...
        if(found_a==true){
          if(found_end==true){
            for(int i=a_position;i<end_position;i++){
              if(gps_data[gps_count-1] == ',' && buffer[i] == ',' ){
                gps_data[gps_count++]='0';
                }
        ...
        char* ptr = strtok(gps_data, delimiter);

        while(ptr != NULL) {
          // Serial.print("found one part:");
           // Serial.println(ptr);
            if(index == 2){
              latitud = atof(ptr);
              }
...
void clearGpsBufferArray()                     // function to clear buffer array
{
    ...
    gps_count=0;// clear all index of array with command NULL
}
...
void clearBufferArray()                     // function to clear buffer array
{
  ...
  count = 0;             // clear all index of array with command NULL
}

void GetAirQuality(){
  air_quality = airqualitysensor.first_vol;
  }
...
void GetAltitude(){
    ...
    temperatura =  t_filter.Filter(t);
    long Pressure = HP20x.ReadPressure();
    t = Pressure/100.0;
    presion= p_filter.Filter(t);
    
    long Altitude = HP20x.ReadAltitude();
    t = Altitude/100.0;
    altitud =  a_filter.Filter(t);
    }
}
...
void SendRadio(){
  Radio.flush();
  Radio.print('#');
  Radio.print(realAccel[0]);
  Radio.print(',');
  Radio.print(realAccel[1]); 
  Radio.print(',');
  Radio.print(realAccel[2]);
  Radio.print(':');
  Radio.print(mag[0]);
  Radio.print(',');
  Radio.print(mag[1]);
  ...
  Radio.print(latitud);
  Radio.print(',');
  Radio.print(latitud_dir);
  Radio.print(',');
  Radio.print(longitud);
  Radio.print(',');
  Radio.print(longitud_dir);
  Radio.print(',');
  Radio.print(gps_altitud);
  Radio.print(':');
  Radio.print(air_quality);
  Radio.println(';');
 }
