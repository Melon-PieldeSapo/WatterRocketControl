...
void AirQuality::avgVoltage()
{
  if(i==150)//sum 5 minutes
  {
    ...
    temp=0;
    Radio.print("Vol_standard in 5 minutes:");     
    Radio.println(vol_standard);   
    i=0;    
  }
    else 
  {
...
void AirQuality::init(int pin)
{
    ...
    unsigned char i=0;
    delay(3000);
    Radio.println("sys_starting...");
    delay(200000);//200000
    init_voltage=analogRead(_pin);

    Radio.println("The init voltage is ...");
    Radio.println(init_voltage);
    while(init_voltage)
    {
        ...
        { 
            i++;
            Radio.println("waitting sensor init..(it takes 60 seconds to init)");
            delay(60000);//60000
...
int AirQuality::slope(void)
{
  while(timer_index)
  {
      if(first_vol-last_vol>400||first_vol>700)
        {
            Radio.println("High pollution! Force signal active.");   
            timer_index=0;  
            avgVoltage(); 
            return 0; 
        }
        ...
        { 
            Radio.print("sensor_value:");    
            Radio.print(first_vol);          
            Radio.println("\t High pollution!");   
            timer_index=0;  
            avgVoltage();
            return 1;
        ...
        {
            //Radio.println(first_vol-last_vol);
            Radio.print("sensor_value:");
            Radio.print(first_vol);          
            Radio.println("\t Low pollution!");    
            timer_index=0;
            avgVoltage();
            return 2; 
        }
...
float sum = 0;
float altura=0;
SoftwareSerial SoftSerial(10, 11); //GPS SERIAl//RX, TX
//DEFINIDO ARRIBA DEL TODO SoftwareSerial Radio(9, 8);//RX, TX
...
{
  char rtn = 0;
    Radio.begin(9600);  // Serial is used for debugging
    delay(2000); //allow serial inicialization;
    Radio.println("\r\npower on");
    SoftSerial.begin(9600);
    delay(2000);
    Radio.println("Radio on");
    rtn = Lsm303d.initI2C();
  ...
  if(OK_HP20X_DEV == ret)
  {
    Radio.println("HP20x_dev is available.\n");    
  }
  else
  {
    Radio.println("HP20x_dev isn't available.\n");
  }
  Radio.println("Starting Airquality");
  airqualitysensor.init(14);
  Radio.println("All ready to go");
...
void loop()
{
 ...
 //Radio.print("0x");
 // Radio.print(Lsm303d.read(IG_SRC2));
 //  Radio.print(" - 0x");
 // Radio.print(Lsm303d.read(IG_SRC1)); //mostrar el valor de la interrupcion (para limpiar el registro)
 //Radio.print(" - ");
 // printValues();
 // printTest();
  //varometer();
 ...
 //Radio.println("bucle principal");
 print_serial();

  delay(500);  // delay for serial readability (delay 200
}

String gps_payload="0,0,0";
  void gps_parsed(){
     if (SoftSerial.available())                     // if date is coming from software serial port ==> data is coming from SoftSerial shield
    {
        while(SoftSerial.available())               // reading data into char array
        {
            ...
            //Radio.print(buffer[count-1]);    // writing data into array
            if(count == 64)break;
        }
        for(int i=0;i<count;i++){
          //Radio.print(buffer[i]);
          //Radio.print(" :- ");
        ...
        //Radio.println("#endbuf");
        if(found_a==true){
          //Radio.print("we found A");
          if(found_end==false){
            //Radio.println(" with no end");
            for(int i=a_position;i<count;i++){
              gps_data[gps_count++]=buffer[i];
            }
          }else{
           // Radio.println(" with end");
          ...
          gps_payload = gps_data;
//          Radio.println(gps_payload);
          int first = gps_payload.indexOf("00,")+3;
          int second = gps_payload.indexOf("E,")+1; //W para espania
//          Radio.println(test.substring(first,second));
//          Radio.println(test.substring(second+5+(test.substring(second+5)).indexOf(",")));
  //TODO: EXTRAER LOS DATOS POR SEPARADO: HACEN FALTA PARA LOS CALCULOS 
  //REVISAR http://wiki.seeed.cc/Grove-GPS/ EL EJEMPLO DE PYTHON! ES MUCHO MA LIMPIO QUE HACER ESTA CHAPUZA DE SUBSTRINGS
  //PROBAR CON SERIAL.READLINE() EN LUGAR DEL FREGADO QUE TENGO MONTADO ARRIBA
          gps_payload=gps_payload.substring(first,second) + gps_payload.substring(second+5+(gps_payload.substring(second+5)).indexOf(","));
...
void varometer(){
    ...
    payload = payload + t_filter.Filter(t);
    long Pressure = HP20x.ReadPressure();
    t = Pressure/100.0;
   payload = payload  +"," + p_filter.Filter(t);
    
    long Altitude = HP20x.ReadAltitude();
    t = Altitude/100.0;
   payload = payload +"," + a_filter.Filter(t) + ":";
    }
}
...
payload = "#";
 payload = payload+realAccel[0] + ",";
 payload = payload+realAccel[1] + ",";
 payload = payload+realAccel[2];
 payload = payload + ":";
varometer();
 gps_parsed(); //se procesa 1 de cada 3 veces
 Radio.print(payload);
 ...
 Radio.println(";");
 //payload = payload+gps_payload;
 //payload=payload +":"+airqualitysensor.first_vol+";";
